...
A user may build a local Python environment based on this module, `python/3.9.13-gpu`, which
...
activate script such that the `python/3.9.13-gpu` module is loaded during activation and unloaded
during deactivation.
...
CMDS="${MARK}\n\n"
CMDS="${CMDS}module -s load python/3.9.13-gpu\n\n"
...
CMDS="${CMDS}${INDENT}module -s unload python/3.9.13-gpu"
