...
These instructions show how to build Miniconda3 environment (based on Python 3.12.1) for the Cirrus GPU nodes
(Cascade Lake, NVIDIA Tesla V100-SXM2-16GB), one that supports parallel computation.

The environment features mpi4py 3.1.6 (OpenMPI 4.1.6 with ucx 1.16.0 and CUDA 12.4) with pycuda 2024.1
and cupy-cuda12x 13.1.0. It also provides a suite of packages pertinent to parallel processing and numerical analysis,
...
Build and install mpi4py using OpenMPI 4.1.6-cuda-12.4
------------------------------------------------------
...
CC=mpicc CXX=mpicxx FC=mpifort CFLAGS="-I${NVHPC_ROOT}/cuda/${CUDA_VERSION}/include" LDFLAGS="-L${NVHPC_ROOT}/cuda/${CUDA_VERSION}/lib64/stubs -L${NVHPC_ROOT}/math_libs/lib64/stubs" pip install pycuda
```
...
A user may build a local Python environment based on this module, `python/3.12.1-gpu`, which
...
activate script such that the `python/3.12.1-gpu` module is loaded during activation and unloaded
during deactivation.
...
CMDS="${MARK}\n\n"
CMDS="${CMDS}module -s load python/3.12.1-gpu\n\n"
...
CMDS="${CMDS}${INDENT}module -s unload python/3.12.1-gpu"
