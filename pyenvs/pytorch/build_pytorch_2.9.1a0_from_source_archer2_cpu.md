Instructions for building PyTorch 2.9.1a0 from source for use on the ARCHER2 CPU nodes
======================================================================================

These instructions show how to build PyTorch 2.9.1a0 for use on the ARCHER2 CPU nodes (HPE Cray EX, AMD Zen2 7742).
...
appropriate HPE MPICH libraries, allowing PyTorch jobs to be run across multiple compute nodes.
...
# PyTorch source version 2.9.1a0 corresponds to PyTorch package version 2.9.1
git checkout v2.9.1
...
`${PYTORCH_BUILD_ROOT}/dist/torch-2.9.1a0+gitd38164a-cp310-cp310-linux_x86_64.whl`.
...
pip install --user --no-deps torchaudio==2.9.1 --extra-index-url https://download.pytorch.org/whl/cpu
pip install --user --no-deps torchvision==0.24.1 --extra-index-url https://download.pytorch.org/whl/cpu
pip install --user --no-deps torchtext==0.18.0 --extra-index-url https://download.pytorch.org/whl/cpu
...
pip install --user lightning-utilities
...
A user may build a local Python environment based on this module, `pytorch/2.9.1a0`, which
...
activate script such that the `pytorch/2.9.1a0` module is loaded during activation and unloaded
during deactivation.
...
CMDS="${MARK}\n\n"
CMDS="${CMDS}module -q load pytorch/2.9.1a0\n\n"
...
CMDS="${CMDS}${INDENT}module -q unload pytorch/2.9.1a0"
